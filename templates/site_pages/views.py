...
def leadership(request):
    sitelook.refresh_from_db()
    if (sitelook.show_leadership):
        leadership = LeadershipHeader.objects.get(id=1)
        leaders = Leader.objects.all()
        lcount = len(leaders)
        row_count = int(lcount // 2)
        if (lcount % 2 == 1):
            last_row_single = True
        else:
            last_row_single = False

        header_image = ('/static/site_pages/leadership/header_image/'
                        + leadership.header_image_file_name)
        ...
        context = {
            ...
            'leaders': leaders,
            'row_count': row_count,
            'last_row_single': last_row_single,
        }
        response = render(request, 'site_pages/leadership.html', context)
        req_time = process_cookie(request, 'Leadership')
        response.set_cookie('req_time', req_time)
        return response
    else:
        return HttpResponseRedirect(reverse('index'))
